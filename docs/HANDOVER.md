...
- **Image Router 配置层（P15 已完成；A1.5 止血已应用）**：Settings → API → Image Router 有两条独立路由（Image Generation / Image Editing），各自绑定 **图片 API 来源 + Model + Enabled + 可选备用通道**。模型下拉的唯一来源是 `assets/js/image-models-core.js`（含 Image 2.5 的真实 id `gpt-image-2.5-flare` / `gpt-image-2.5-sunburst`，按 `image-generation` / `image-editing` 能力过滤）；路由配置存 `apiSettings['image_router'].routes`（与既有并发覆盖字段同 key）。解析语义：`apiConfigId` 留空 = 跟随当前角色（旧行为不变），有值 = 该路由固定用这条来源；`model` 留空 = 交回 Fast/Auto/Precision 双模型策略，有值 = 显式优先（Fast/Precision 不能改它）。配置问题（路由关闭 / 绑定配置不存在 / 模型不支持 / 公网端点缺 Key）在**发请求前**给出明确错误码与「去哪修」的文案，0 次 provider 请求；本地/内网端点允许不填 Key。**A1.5 变更**：两条路由不再有「+ 新建」按钮（IB 没有独立的图片 API 配置实体，那条记录同时就是角色档案，点下去打开的是角色 API 编辑器 → 语义误导），文案从 "API Config" 改为「图片 API 来源」，inherit 显示「跟随当前角色」，并说明图片凭据来自角色 API 配置里的 图片服务商 / 图片接口地址 / 图片 API Key；新建来源的唯一入口是「角色库」的 API 编辑器。**路由 schema / executor / P4 `apiConfigs` 契约三者零改动**。
...
**当前阶段（U5-6 · v1.0.4 已发布）：** `v1.0.4` 已按 RELEASE.md §2 顺序上线（tag `v1.0.4` → `8780bfa`，显式 HEAD；三个资产 digest 与清单交叉核对通过；Stable 通道已生效——`1.0.3` 客户端读到 `update-available → 1.0.4`、`1.0.4` 读到 `up-to-date`，且这次 primary 直连 **200 一次成功、没有触发备路**）。上游差异就是四项代码改动：A1.5 止血（`643954b`）、P11-3 Middle Brain runtime semantics closure（`4107bd5`）、P21 统一思考深度（`75325ae`）、P21.1 DeepSeek `deepseek-flash` 能力校准（`755b666`）；**发布契约零增量**。构建与发布实测见 [RELEASE.md](RELEASE.md) §8，用户升级说明见 [release-notes/1.0.4.md](release-notes/1.0.4.md)，逐条记录见 [CHANGELOG.md](CHANGELOG.md)。上一版（1.0.3）的安装态复核四项验收全部通过（版本号三处一致、`glass-ripple.js` = 10,687 B / `sha256=1dc0a3f3…8b51` 与仓库 HEAD 及 HTTP 下发字节三者逐字节相同、`bg-canvas.jpg` 200 / `.png` 404、安装器自动重启，水纹由用户目视确认）。

**仍唯一未结项的一跳**：应用内零触达的「卡片点击 → 下载 → 静默安装」半程（`runtime/update-install.js` + 诊断卡片到助手的那一跳）**至今仍未被真实点击验证**——1.0.1 / 1.0.2 / 1.0.3 三次升级都是手动双击安装包（安装器日志无 `/SILENT`、无 `IBRELAUNCH`，`%LOCALAPPDATA%\InternalBeyond\updates` 不存在）。**1.0.4 是 1.0.3 之后第一个有真实功能变更的版本，正是补这一跳的时机**：在真实安装实例（`E:\IB-E2E-1.0.1\InternalBeyond`，现为 1.0.3）的诊断页点一次「下载并安装」即可——检查半程已在本次发布后用真实客户端路径跑通（只读，缓存写临时文件），缺的只有下载 + 静默安装 + 自动重启。该实例升级后即变为 1.0.4，**不要卸载**。U5-2B（legacy root-layout cleanup / `[InstallDelete]`）仍是独立后续项，本阶段不做。
...
- **应用内零触达的「安装半程」未验证（U5-4 收尾留档，2026-09-10）**：`runtime/update-install.js` 的「读缓存清单 → 下载 → 大小 / SHA-256 / PE 版本校验 → detached 拉起安装器」，以及诊断卡片到助手那一跳，**至今没有在真实点击下跑过**——当天的三次 1.0.x 升级全部是用户从浏览器下载安装包后双击覆盖安装（安装器日志无 `/SILENT`、无 `IBRELAUNCH`，`Original Setup EXE` 指向 `%USERPROFILE%\Downloads`；`%LOCALAPPDATA%\InternalBeyond\updates` 从未出现）。**检查半程已通**（真实安装态 `GET /__update-check?force=1` → `transport=direct` / `latestVersion=1.0.3`，缓存刷新为 1.0.3；同一实例的自动路径暴露了「陈旧清单在 24h TTL 内遮蔽新版本」——只会少报，不会误报有更新）。要真正闭环需要一个**有真实变更**的新版本，不为测试单独发版，详见 [RELEASE.md](RELEASE.md) §8。
...
- Image Router UX 止血（A1.5）已知限制：① **Image API Config 与 Character 仍未正式解耦**——`apiConfigs` 一条记录同时是"API 配置"与"角色档案"，所以 Router 的"图片来源"只能指向角色记录（`inherit` 与"绑定某条记录"在当前数据模型下是同一张表的两个 id）。本轮只做界面止血（去掉「+ 新建」、文案改「图片 API 来源」/「跟随当前角色」并说明凭据来源），**没有**引入独立 `image_configs`、**没有**拆表、**没有**改 generation/editing path schema、**没有**动 executor 与 producer gate —— 这四项整体登记为 **P16 · Image Config Decoupling**；② 该止血**不解除角色级闸门**：绑定了路由来源但角色没勾「启用图像生成」仍会被拦在 `workspace.js` 的 `if(!cfg.imageGen)`（P15 限制①仍然成立，正式解耦时要一并处理）；③ API 页顶级卡片间距现在是结构性 contract（`api-components.css` 的 `#page-api > .glass-card` / `#page-api > .api-section{margin-bottom:28px}`，修掉"系统诊断入口与 Middle Brain 贴死 0px"），**新增顶级卡不需要再自己加 margin**，但也不得用 nth-child / 负 margin / Middle Brain 特判打补丁（`test_frontend_structure.js` 的 `apiPage.rhythm*` 两条守卫锁死，真实间距与折叠不变性由 `test_image_router_settings_smoke.js` 的 `spacing.*` 断言守）；④ 新建 API 配置现在逐字段清空全部五个图片字段（此前只清 toggle/model，会把上一个角色的图片接口地址与图片 API Key 静默带进新角色）——回归由 `test_api_key_mutation_repro.js` 的 M9–M12 守。
