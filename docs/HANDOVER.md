...
**当前阶段（U5-5 · v1.0.3 发布准备）：** `v1.0.2` 已按 RELEASE.md §2 顺序上线（tag `v1.0.2` → `7e86c5e`，三个资产 digest 与清单交叉核对通过，Stable 通道已生效）。`1.0.1 → 1.0.2` 的更新链本身已跑通（真实安装实例升到 1.0.2、欢迎页背景恢复），但收尾核验时发现**新的真实安装态回归**：欢迎页的动态水纹特效整片不显示，因此 **U5-4 未给最终 PASS**。根因已定位（R0）并修复：`assets/js/glass-ripple.js` 的 `idleNow()` 里有一条「离开欢迎页（`currentPage !== 'home'`）后暂停逐帧水波模拟」的守卫，深链进入内容页（如 `#diagnostics`）而欢迎页仍可见时它会把整帧渲染掐掉；该守卫自 v1.0.0（`6a61c3c`）起就存在，只是被 1.0.2 之前「背景不显示」的问题掩盖。修复提交 `cbd798f` 已 push。**`v1.0.3` 已按 RELEASE.md §2 顺序上线**（tag `v1.0.3` → `666adc0`，三个资产 digest 与清单交叉核对通过，Stable 通道已生效：`1.0.2` 客户端读到 `update-available`）。用户升级说明见 [release-notes/1.0.3.md](release-notes/1.0.3.md)。**安装态复核已完成（U5-4 收尾）**：四项验收全部通过——`/health` / `VERSION` / `boot-state.json` / 卸载注册表全为 `1.0.3`（全机仅一项安装）；安装目录 `assets/js/glass-ripple.js` = 10,687 B / `sha256=1dc0a3f3…8b51`，与仓库 HEAD 及 HTTP 下发字节三者逐字节相同且不含 `currentPage`；`bg-canvas.jpg` 200 / `bg-canvas.png` 404；安装器停旧版本后自动用 `?ibv=1.0.3` 重启；水纹由用户目视确认。**但该次升级是手动下载安装包双击覆盖安装**（安装器日志显示 `Downloads\InternalBeyond-Setup-1.0.3.exe`、无 `/SILENT` / `IBRELAUNCH`；`%LOCALAPPDATA%\InternalBeyond\updates` 不存在），所以「卡片点击 → 下载 → 静默安装」这半程**仍未在真实点击下验证**——它目前只有测试与 harness 覆盖，记为独立未结项，等下一个有真实变更的版本自然做，不为测试单独发版（检查半程已通：`/__update-check?force=1` → `direct` / `1.0.3`，缓存刷新为 1.0.3）。baseline 实例现在是 1.0.3，**不要卸载**。U5-2B（legacy root-layout cleanup / `[InstallDelete]`）仍是独立后续项，本阶段不做。
