...
### 5.1 发行声明闸门（在第 4 步，不在第 7 步）

上面 6 条管的是**清单与 exe 字节是否对得上**，所以都在第 7 步。另有一道闸门位置更早
（第 4 步的载荷审计），因为它管的是**载荷内容本身**：

> `README.md` 在载荷白名单里（见 [release-manifest.js](../scripts/release-manifest.js)），
> 所以它向用户声明「当前版本是 X」这句话，**本身就是已发布产物的一部分**。
> `scripts/release-audit.js` 的 `scanReleaseClaims` 因此要求：暂存载荷里 README 声明的
> 「当前版本」必须等于同一份载荷里的 `VERSION`；不得硬编码指向别的版本的安装包名；
> 锚点消失（声明被改写或删掉）同样判失败——**fail closed，绝不静默放行**。

这条闸门是 `v1.0.4` 之后补的，起因是一次真实的漏改：`1.0.1` / `1.0.2` / `1.0.3` 的发行
提交都在同一个提交里同步了 README，而 **`1.0.4` 的发行提交只改了 `VERSION` 与发行文档**，
于是落地页与**安装包内部**那份 README 都还写着 1.0.3、并引导用户去下载它，而 Releases 的
Latest 已经是 1.0.4。**没有任何检查失败——是人读页面时发现的。**

配套地，README 现在只保留**一处**需要手改的版本字面量（开头的「当前版本 **x.y.z**」），
其余安装包名一律写成 `InternalBeyond-Setup-<版本号>.exe` 占位符：把每次发行的漏改面
从 6 处降到 1 处，而这一处由上面的闸门兜住。
