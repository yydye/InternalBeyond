...
## 2026-09-11 · 上游署名被搬离 README 顶部：恢复位置 + 位置闸门

用户报「README.md 里原作者 Sui 的说明被删了」。**核查结论：没有人删过它。** 署名段今天仍在
`README.md`：「关于本仓库 / About this fork」与「许可与版权 · 衍生版本说明」两处并存，
连 `c151bb1`（上游 baseline）的「联系方式」与「许可与版权」两节也逐字保留；变化只有标题层级
（`### ✦` → `##`）与链接路径随 `e612197` 的目录重组而变。真正发生的是**位置被移了**：

| 时间 | 提交 | 署名段在 README 的第几行 | README 总行数 | HANDOVER 那句守卫怎么写 |
|---|---|---|---|---|
| 2026-08-26 | `bbafba5` | **第 11 行**（顶部，紧随简介） | 379 | 「README **顶部**「关于本仓库」…改动 README 时勿删除这两处」 |
| 2026-09-09 | `23c8960` | **第 258 行**（约文末 87% 处） | 298 | 「README **的**「关于本仓库 / About this fork」…」 |

`23c8960`（`release: finalize Windows installer and public release docs`）把 README 从 623 行重写成
298 行的「下载优先」文档，署名段整体搬到文末——**并在同一次提交里，把守卫那句中的「顶部」删掉了**。
守卫先松、内容后移，是同一个提交干的，所以之后没有任何东西再把位置当回事。

后果不是观感问题：上游授权要求「在**显著位置**说明修改内容」，而 README 是公开落地页，
把署名压到 87% 处等于让它在页面上方消失。

处理：

1. `README.md` 顶部（第 8–9 行，紧随版本行）**恢复**一段中英署名声明，指向文末完整段落；
   文末两处原样保留，一个字未删（**恢复位置，不是移动内容**）。
2. `docs/HANDOVER.md` 的守卫改回**位置约束**：「README **顶部**（前 25 行内那段署名声明，
   由 `scripts/release-audit.js` 的发行声明闸门强制）…改动 README 时勿删除、勿下移」。
3. 闸门扩展：`scanReleaseClaims` 增加两条 fail-closed 检查——署名声明必须落在 README 前
   25 行内（否则报「被移出了 README 前 25 行」，而不是静默通过），且「## 关于本仓库 /
   About this fork」小节必须存在。
4. 反向自测（不做等于没加）：用**真实暂存载荷**的 README 复现 `23c8960` 的形状（把顶部那段
   挪到文末）→ `ok=false` 并报「上游署名被移出了 README 前 25 行」；把小节标题改名 →
   报「缺少『## 关于本仓库 / About this fork』小节」。夹具侧 `tests/test_installer.js` 新增一项，
   覆盖「顶部通过 / 埋到 30 行之后失败 / 删除失败」。

教训与上一条同源：**写在文档里的散文守卫挡不住一次重写。位置约束必须由构建闸门钉住。**
