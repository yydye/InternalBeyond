  ...
  var CHAPTERS = [
    ...
    },
    {
      ...
      shots: [
        ...
        { id: '08-api-entry', caption: 'API 设置页：顶部是获取 API Key 的入口，下方是「+ 添加API」' }
      ],
      steps: [
        '打开左侧导航的「API」。',
        '还没拿到 Key：点顶部的「带我获取」，选一个你要用的服务，按提示的 3–5 步在你所用服务的官方网站获取一把 Key，然后点「配置到 IB」——服务、地址和模型会自动填好，你只需要粘贴 Key。',
        '已经拿到 Key：点「去粘贴」，同样会自动填好，你只需要粘贴 Key。',
    ...
    },
    {
      ...
      items: [
        ...
        { q: '我还没有 API Key，从哪里开始？', a: '打开「API」页，点顶部的「带我获取」：先选一个服务，照着 3–5 步在服务商的官方网站获取一把 Key，再点「配置到 IB」，服务、地址和模型都会自动填好，你只要粘贴 Key。' },
        { q: '别人卖我的中转 Key 能用吗？', a: '能，但那是第三方服务，不是模型官方。点「导入 OpenAI Compatible API」把对方给你的地址和 Key 填进去即可。价格、余额和稳定性都由那家第三方决定，IB 只负责接上，不为它背书。' },
