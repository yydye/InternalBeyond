...
async function resetAllData(){
  ...
  const stores=['posts','categories','about','music','apiSettings','chatMessages','letters','apiConfigs','groups','blogComments','memories','chatThreads','chatSummaries','uploadedFiles','projects','projectFiles','blogAnnotations','active_message_settings','active_message_history','active_message_plans','diary_entries','roleLetters'];
...
const _IMGGEN_INSTR_BLOCK='\n\n【图像生成】本对话已开启图像生成能力。当用户希望你画图/生成图片时，输出：\n<ws_gen_image prompt="画面的完整描述" size="1024x1024"/>\n规则：prompt 写清晰具体的画面描述（主体、风格、构图、光线、色调等，中英文皆可）；size 可选且仅支持枚举值 1024x1024、1536x1024、1024x1536（不写则用默认——生图接口不接受 200x200 之类的任意尺寸，用户要小图时请照常按默认尺寸生成，并说明图片可自行缩放使用）；描述很长时也可写成 <ws_gen_image>描述…</ws_gen_image> 的正文形式；每条回复最多 1 个生图标签。生成的图片会直接显示在聊天中并自动存入 ICode，生成结果会在之后的消息回传给你——你可以直接看到生成结果并做出评价，无需再用 ws_read_image 自行读取。不要在文字里假装已生成——没有标签就没有图片。';
