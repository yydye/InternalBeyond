...
**正式发行文件：`InternalBeyond-Setup-<版本号>.exe`（当前 1.0.4，约 49 MB）**
...
1. 在 [Releases 页面](https://github.com/yydye/InternalBeyond/releases) 下载 `InternalBeyond-Setup-1.0.4.exe`。
...
- 从 **1.0.0**（或更早）升级，需要**手动安装一次最新版**：在 [Releases 页面](https://github.com/yydye/InternalBeyond/releases) 下载 `InternalBeyond-Setup-1.0.4.exe` 并运行（覆盖安装即可）。早期版本还没有应用内更新的能力，所以这一跳必须手动走一次。
...
`InternalBeyond-Setup-1.0.4.exe`. No Node.js, npm, Python, PowerShell or admin rights required;
...
Upgrading from **1.0.0** (or earlier) requires **one manual install** of the current release: download `InternalBeyond-Setup-1.0.4.exe` from Releases and run it. Earlier versions predate in-app updates, so that single hop has to be done by hand; every release from **1.0.2** on can be installed from inside the app.
