...
**正式发行文件：`InternalBeyond-Setup-<版本号>.exe`（约 49 MB，`<版本号>` 即本文开头标注的当前版本）**
...
1. 在 [Releases 页面](https://github.com/yydye/InternalBeyond/releases) 下载标着 **Latest** 的 `InternalBeyond-Setup-<版本号>.exe`。
...
- 从 **1.0.0**（或更早）升级，需要**手动安装一次最新版**：在 [Releases 页面](https://github.com/yydye/InternalBeyond/releases) 下载最新版 `InternalBeyond-Setup-<版本号>.exe` 并运行（覆盖安装即可）。早期版本还没有应用内更新的能力，所以这一跳必须手动走一次。
...
```bash
node tests/test-all.js --quick     # 静态结构 + 本地服务（无浏览器，本机实测约 217 秒）
...
| [`DECISIONS.md`](docs/DECISIONS.md) | 为什么这么设计（D1–D19，"不要随便改"清单） |
...
`InternalBeyond-Setup-<version>.exe`. No Node.js, npm, Python, PowerShell or admin rights required;
...
Upgrading from **1.0.0** (or earlier) requires **one manual install** of the current release: download the latest `InternalBeyond-Setup-<version>.exe` from Releases and run it. Earlier versions predate in-app updates, so that single hop has to be done by hand; every release from **1.0.2** on can be installed from inside the app.
