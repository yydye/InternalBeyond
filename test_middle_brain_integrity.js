...
(async () => {
  ...
  try {
    ...
    const comText = fs.readFileSync(path.join(__dirname, 'assets', 'js', 'communication.js'), 'utf8');
    ...
    check('A10.noOtherConsumer', !fs.readdirSync(path.join(__dirname, 'assets', 'js')).filter(f => f.endsWith('.js'))
      .filter(f => !/^middle-brain/.test(f) && f !== 'communication.js').some(f => /middleBrainFinalizeReply\s*\(/.test(fs.readFileSync(path.join(__dirname, 'assets', 'js', f), 'utf8'))));
    ...
    const ciSrc = fs.readFileSync(path.join(__dirname, 'assets', 'js', 'middle-brain-integrity.js'), 'utf8');
