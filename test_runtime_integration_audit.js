...
const createHttp = require('./active/http.js');
...
(async () => {
  ...
  try {
    ...
    await check('Opt-in Runtime resolves Anthropic through shared directory', async () => {
      const { context } = environment(request);
      context.IBModelCore = require('./assets/js/ib-model-core.js');
      context.PROVIDERS = require('./assets/js/provider-directory.js').PROVIDERS;
