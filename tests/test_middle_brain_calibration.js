...
function readLayer(rel) { return fs.readFileSync(path.join(ROOT, rel), 'utf8').replace(/^\uFEFF/, ''); }
...
function loadProductionSandbox(opts) {
  ...
  const providerDir = require(path.join(ROOT, 'assets', 'js', 'provider-directory.js'));
...
check('H4.noProductionFileModified', (() => {
  const before = fs.readFileSync(path.join(ROOT, 'assets', 'js', 'middle-brain-integrity.js'), 'utf8');
  CAL.runCalibration(CASES, deps);
  return fs.readFileSync(path.join(ROOT, 'assets', 'js', 'middle-brain-integrity.js'), 'utf8') === before;
})());
check('H5.harnessNotLoadedInHtml', !/middle-brain-calibration/.test(fs.readFileSync(path.join(ROOT, 'InternalBeyond.html'), 'utf8')));
check('H6.harnessNotInProductionAssets', !fs.existsSync(path.join(ROOT, 'assets', 'js', 'middle-brain-calibration.js'))
...
check('H7.noProductionFileReferencesHarness', (() => {
  ...
  (function walk(d) {
    for (const e of fs.readdirSync(d, { withFileTypes: true })) {
      ...
      else if (/\.js$/.test(e.name) && /middle-brain-calibration/.test(fs.readFileSync(p, 'utf8'))) hits.push(path.relative(ROOT, p));
    }
  })(dir);
