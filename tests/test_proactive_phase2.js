...
/* 数据目录隔离（P7 测试隔离契约）：必须在 require(path.join(ROOT, 'services', 'active-message-service.js')) 之前
...
process.env.IB_ACTIVE_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), 'ib-proactive-p2-'));
...
const createProactiveTrace = require(path.join(ROOT, 'active', 'proactive-trace.js'));
