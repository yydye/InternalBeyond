...
{
  const ui = fs.readFileSync(path.join(ROOT, 'assets', 'js', 'backend-restart.js'), 'utf8');
...
(async () => {
  ...
  let runnerOut = '';
  const runner = spawn(process.execPath, [RUNNER], { cwd: ROOT, env, stdio: ['ignore', 'pipe', 'pipe'] });
     ...
     the restart must report failed (EADDRINUSE on the occupied port). */
  {
    ...
    let runner2Out = '';
    const runner2 = spawn(process.execPath, [RUNNER], { cwd: ROOT, env: env2, stdio: ['ignore', 'pipe', 'pipe'] });
