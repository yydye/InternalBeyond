...
function loadCall() {
  ...
  vm.runInContext(fs.readFileSync(path.join(ROOT, 'assets/js/communication/call.js'), 'utf8'), sandbox, { filename: 'call.js' });
...
check('voice-worklet.js posts the {pcm,rms} contract onCapture expects', () => {
  const src = fs.readFileSync(path.join(ROOT, 'assets/js/voice-worklet.js'), 'utf8');
